"""
DLAMA-v1 Results Visualization Script
Analyzes Arab vs Western culture evaluation results for Qwen 2.5-3B
"""
...
def create_arab_vs_western_table(df: pd.DataFrame, metric: str = 'overlap') -> pd.DataFrame:
    """
    Create a simple 2x2 table: Arab vs Western overall accuracy
...
def plot_arab_vs_western_table(df: pd.DataFrame, metric: str = 'overlap', model_name: str = 'Qwen 2.5-3B'):
    """Create visual table for Arab vs Western comparison"""
    
    table_df = create_arab_vs_western_table(df, metric)
    ...
    ax.set_title(
        f'DLAMA-v1: Arab vs Western Culture Comparison\n'
...
def plot_top_bottom_predicates_by_culture(df: pd.DataFrame, metric: str = 'overlap', 
    ...
    colors_arab = '#e74c3c'
    colors_western = '#3498db'
    
    for culture_idx, culture in enumerate(['Arab', 'Western']):
        culture_df = df[df['culture'] == culture]
...
def create_summary_statistics_table(df: pd.DataFrame, metric: str = 'overlap', 
    ...
    for culture in ['Arab', 'Western']:
        ...
        for country in culture_df['country'].unique():
            country_df = culture_df[culture_df['country'] == country]
...
def main():
    ...
    # Configuration
    results_file = Path("results/dlama/your_experiment_timestamp.json")
    output_dir = Path("results/dlama/visualizations")
    ...
    print("\n" + "="*70)
    print("🔍 DLAMA-v1 Results Visualization")
    print("="*70)
    ...
    # 1. Arab vs Western simple table
    print("   1. Arab vs Western comparison table...")
    fig1 = plot_arab_vs_western_table(df, metric, model_name)
    fig1.savefig(output_dir / f'1_arab_vs_western_table_{metric}.png', 
    ...
    # Arab vs Western table
    arab_west_df = create_arab_vs_western_table(df, metric)
    arab_west_df.to_csv(output_dir / f'arab_vs_western_{metric}.csv', index=False)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Visualize DLAMA-v1 results")
    parser.add_argument(
        "--results_file",
        type=str,
        required=True,
